...
val data = spark.read.option("inferSchema", "true").option("header", "true").csv("Wholesale customers data.csv")
...
val assembler = new VectorAssembler().setInputCols(Array("Fresh", "Milk", "Grocery",
...
val km = new KMeans().setK(12).setSeed(12345)

// Fit that model to the training_data
val km_model = km.fit(training_data)

// Evaluate clustering by computing Within Set Sum of Squared Errors.
val WSSSE = km_model.computeCost(training_data)
