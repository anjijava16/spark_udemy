...
val cancerdata = spark.read.option("inferSchema", "true").option("header", "true").csv("Cancer_Data")

// Print the Schema of the data

cancerdata.printSchema()
...
val cdfeatures = assembler.transform(cancerdata).select($"features")
...
val scalerModel = scaler.fit(cdfeatures)

// Normalize each feature to have unit standard deviation.
// Use transform() off of this scalerModel object to create your scaledData
val scaledData = scalerModel.transform(cdfeatures)
...
val pcaDF = pcaModel.transform(scaledData)
