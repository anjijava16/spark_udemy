...
val df = data.select(data("Clicked on Ad").as("label"), $"Daily Time Spent on Site", $"Age", $"Area Income", $"Daily Internet Usage", $"Male", $"Timestamp")

val df2 = df.withColumn("Hour", hour(df("Timestamp")))
...
val results = pipeline_model.transform(test)
